...
epochs = 50  # 训练迭代次数
model_name = "VGG"  # 模型名称"AlexNet"、"VGG"、"Googlenet","Resnet","MobilenetV2","MobilenetV3"、"Shufflenet"、"EfficientnetV1","EfficientnetV2"
...
for epoch in range(1, epochs + 1):
    train_loss.reset_state()  # clear history info
    train_accuracy.reset_state()  # clear history info
    test_loss.reset_state()  # clear history info
    test_accuracy.reset_state()  # clear history info
    ...
    if test_loss.result() < best_test_loss:  # 保存损失最小的模型
        ...
        model.save_weights(f"./save_weights/my{model_name}.weights.h5")
