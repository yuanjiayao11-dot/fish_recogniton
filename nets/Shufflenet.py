...
def shuffle_block_s1(inputs, output_c: int, stride: int, prefix: str):
    ...
    x1, x2 = ChannelSplit(name=prefix + "_split")(inputs)

    # main branch
    x2 = ConvBNReLU(filters=branch_c, name=prefix + "_b2_conv1")(x2)
    x2 = DWConvBN(kernel_size=3, strides=stride, name=prefix + "_b2_dw1")(x2)
    x2 = ConvBNReLU(filters=branch_c, name=prefix + "_b2_conv2")(x2)

    x = layers.Concatenate(name=prefix + "_concat")([x1, x2])
    x = ChannelShuffle(x.shape, name=prefix + "_channelshuffle")(x)
...
def shuffle_block_s2(inputs, output_c: int, stride: int, prefix: str):
    ...
    # shortcut branch
    x1 = DWConvBN(kernel_size=3, strides=stride, name=prefix + "_b1_dw1")(inputs)
    x1 = ConvBNReLU(filters=branch_c, name=prefix + "_b1_conv1")(x1)

    # main branch
    x2 = ConvBNReLU(filters=branch_c, name=prefix + "_b2_conv1")(inputs)
    x2 = DWConvBN(kernel_size=3, strides=stride, name=prefix + "_b2_dw1")(x2)
    x2 = ConvBNReLU(filters=branch_c, name=prefix + "_b2_conv2")(x2)
